...
class TelemetryManager {
    ...
    maxEvents = 1000;
    constructor(cxfDir) {
        this.telemetryDir = path_1.default.join(cxfDir, 'telemetry');
        this.logFile = path_1.default.join(this.telemetryDir, 'events.jsonl');
    }
    ensureDir() {
        if (!fs_1.default.existsSync(this.telemetryDir)) {
            fs_1.default.mkdirSync(this.telemetryDir, { recursive: true });
        }
    }
    /**
     ...
     */
    async logTelemetryEvent(taskId, intent, finalObjects, totalTokens, maxBudget) {
        try {
            this.ensureDir();
            ...
            for (const obj of finalObjects) {
                ...
                if (sourceLower.includes('rules'))
                    rulesTokens += tokens;
                else if (sourceLower.includes('memory'))
                    memoryTokens += tokens;
                else if (sourceLower.includes('knowledge'))
                    knowledgeTokens += tokens;
                else
                    runtimeTokens += tokens;
                // Domain matching check
                const objectTags = obj.metadata.tags || [];
                const isDomainMatch = intent.domains.some(d => objectTags.includes(d.toLowerCase()) || sourceLower.includes(d.toLowerCase()));
                if (isDomainMatch) {
            ...
            if (intent.riskLevel === 'medium')
                successRate += 15;
            if (totalTokens > 0 && totalTokens <= 3000)
                successRate += 15;
            ...
            const matchingPastEvent = pastEvents.reverse().find(e => e.intent.domains.join(',') === intent.domains.join(','));
            ...
            await fs_1.default.promises.appendFile(this.logFile, JSON.stringify(event) + '\n', 'utf-8');
    ...
    pruneEventsLogIfNeeded() {
        try {
            ...
            if (lines.length > this.maxEvents) {
                ...
                fs_1.default.writeFileSync(this.logFile, trimmed.join('\n') + '\n', 'utf-8');
            }
        }
        catch (e) {
            // Non-fatal
